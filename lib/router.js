...
var handleResourceIndex = function(modelName, query, callback) {
  ...
  if (!!query) {
    where = { where: query };
    if (!!query.json) {
      // where = { where: JSON.parse(query.json) };
      // we now need to place 'where' key inside JSON.stringify and enocdeURI
      // but now we can use other keys like 'offset' and 'limit'
      // example:
      // > encodeURI(JSON.stringify({where:["email LIKE \"%gmail%\""], limit:1}))
      //  "%7B%22where%22:%5B%22email%20LIKE%20%5C%22%25gmail%25%5C%22%22%5D,%22limit%22:1%7D"
      // try this by opening browser at:
      //  http://localhost:3000/api/Users?json=%7B%22where%22:%5B%22email%20LIKE%20%5C%22%25gmail%25%5C%22%22%5D,%22limit%22:1%7D
      where = JSON.parse(query.json);
    }
  }
  console.log(where);
  console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! ");
